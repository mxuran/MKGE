...
trainer.run()
if add_mol:
	distmult.save_checkpoint('./checkpoint_'+dataset+'/distmult_my.ckpt')
else:
	distmult.save_checkpoint('./checkpoint_'+dataset+'/distmult.ckpt')
...
# 读取模型
if add_mol:
	distmult.load_checkpoint('./checkpoint_'+dataset+'/distmult_my.ckpt')
else:
	distmult.load_checkpoint('./checkpoint_'+dataset+'/distmult.ckpt')
...
""" 4、DDI分类"""
tester = Tester(model = distmult, data_loader = None, use_gpu = True)
