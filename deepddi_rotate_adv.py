...
# ----------------------------------------------训练----------------------------------------
""""""
trainer = Trainer(model = model, data_loader = train_dataloader, train_times = 400, alpha = 1e-4, use_gpu = True, opt_method = "adam")
trainer.run()
if add_mol:
...
# 读取模型
if add_mol:
	rotate.load_checkpoint('./checkpoint_'+dataset+'/rotate_my.ckpt')
else:
