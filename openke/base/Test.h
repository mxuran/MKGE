...
/*
extern "C"
void getTest(INT *ph, INT *pt, INT *pr) {
...
extern "C"
void testRel(REAL *con, INT lastRel) {
    ...
    //minimal是真实值的分数，如果小于该值，说明预测为真，那么对应的真实值的位次就后移，这意味着rel_s为真实值的排名
    ...
    //rel_filter_s小于10，表示预测的关系hits@10, 则hits@10总数的对应值rel_filter_tot+1
    ...
    //rel_filter_rank 计算MR值的，将所有排名相加，得到总的
    rel_filter_rank += (rel_filter_s+1);
    rel_rank += (1+rel_s);
    //rel_filter_reci_rank 计算MRR值的，将所有排名相加，得到总的
...
void test_relation_prediction() {
    printf("test_relation_prediction:\n");
    rel_rank /= testTotal;
...
extern "C"
//返回所有测试数据到python
void getTestList(INT *ph, INT *pt, INT *pr) {
    for (INT i = 0; i < testTotal; i++) {
        ph[i] = testList[i].h;
        pt[i] = testList[i].t;
        pr[i] = testList[i].r;
